//
//  GesturePassworViewController.m
//  GestureLockDemo
//
...
@interface GesturePasswordController () <GesturePasswordDelegate, UIAlertViewDelegate>

@property (strong, nonatomic) GesturePassworView *gestureLockView;
...
- (void)setupMainUI {
    ...
    // 账户头像
    UIImageView *headIcon = [[UIImageView alloc] initWithFrame:CGRectMake(0,0, 56, 56)];
    
    headIcon.image = [UIImage imageNamed:@"GesturePassword.bundle/gesture_headIcon@2x.png"];
    ...
    [nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(headIcon.mas_bottom).offset(5);
        make.centerX.equalTo(self.view);
    }];
    ...
    // 九宫格 手势密码页面
    GesturePassworView *gestureLockView = [[GesturePassworView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.width)];
    ...
    [gestureLockView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(weakSelf.view);
        make.left.equalTo(weakSelf.view.mas_left);
        make.right.equalTo(weakSelf.view.mas_right);
        make.top.equalTo(weakSelf.statusLabel.mas_bottom).offset(10);
        make.height.mas_equalTo(self.view.frame.size.width > self.view.frame.size.height?self.view.frame.size.height:self.view.frame.size.width  );
    }];
...
- (void)createGesturesPassword:(NSMutableString *)gesturesPassword {
    ...
    if ([self.lastGesturePsw isEqualToString:gesturesPassword]) { // 绘制成功
        ...
        [self dismissViewControllerAnimated:YES completion:^{
            // 保存手势密码
            [GesturePasswordController addGesturesPassword:gesturesPassword];
        }];
...
- (void)validateGesturesPassword:(NSMutableString *)gesturesPassword {
    ...
    if ([gesturesPassword isEqualToString:[GesturePasswordController gesturesPassword]]) {
...
#pragma mark - GesturePassworViewDelegate
